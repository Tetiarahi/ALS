        ...
        <div class="col-md-12">
            @can('qualification_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.qualifications.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $qualification->id }}">
                                        ...
                                        <td>
                                            @can('qualification_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.qualifications.show', $qualification->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('qualification_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.qualifications.edit', $qualification->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('qualification_delete')
                                                <form action="{{ route('frontend.qualifications.destroy', $qualification->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('qualification_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.qualifications.massDestroy') }}",
